...
class TdnnAffine(nn.Module):
    def __init__(self, input_dim, output_dim, context=[0], bias=True, pad=True, stride=1, groups=1):
        ...
        self.weight = nn.Parameter(torch.randn(output_dim, input_dim // groups, self.tot_context))
    ...
    def forward(self, x):
        # x: [B, C, T]
        if self.pad:
            x = F.pad(x, (-self.left_context, self.right_context), mode="constant", value=0.0)
...
class AttentionAlphaComponent(nn.Module):
    def __init__(self, input_dim, num_head=1, split_input=True, share=True, affine_layers=2, hidden_size=64, context=[0], bias=True):
        super(AttentionAlphaComponent, self).__init__()
        self.input_dim = input_dim
        if num_head > 1 and split_input: assert input_dim % num_head == 0
        
        if share: final_dim = 1
        elif split_input: final_dim = input_dim // num_head
        else: final_dim = input_dim

        self.relu_affine = (affine_layers == 2)
        if self.relu_affine:
            self.first_affine = TdnnAffine(input_dim, hidden_size * num_head, context=context, bias=bias, groups=num_head if (num_head > 1 and split_input) else 1)
            self.relu = nn.ReLU(inplace=True)
            last_in_dim = hidden_size * num_head
        else:
            last_in_dim = input_dim

        self.last_affine = TdnnAffine(last_in_dim, final_dim * num_head, context=context, bias=bias, groups=num_head if (num_head > 1 and split_input) else 1)
...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def __init__(self, input_dim, affine_layers=2, hidden_size=64, context=[0], stddev=True, **kwargs):
        super(AttentiveStatisticsPooling, self).__init__()
        self.stddev = stddev
        self.input_dim = input_dim
        self.output_dim = 2 * input_dim if stddev else input_dim
        self.attention = AttentionAlphaComponent(input_dim, num_head=1, share=True, affine_layers=affine_layers, hidden_size=hidden_size, context=context)
...
class MultiHeadAttentionPooling(nn.Module):
    ...
    def __init__(self, input_dim, num_head=4, stddev=True, affine_layers=1, **kwargs):
        ...
        self.stddev = stddev
        self.output_dim = 2 * input_dim if stddev else input_dim
        self.attention = AttentionAlphaComponent(input_dim, num_head=num_head, split_input=True, share=True, affine_layers=affine_layers)
...
def get_pooling_layer(config, input_dim):
    ...
    if ptype in ['tap', 'mean']:
        ...
        return AttentiveStatisticsPooling(input_dim, stddev=stddev, 
                                          hidden_size=getattr(config, 'att_hidden_size', 64))
                                          
    elif ptype in ['mha', 'multihead']:
        return MultiHeadAttentionPooling(input_dim, stddev=stddev, 
                                         num_head=getattr(config, 'heads', 4))
